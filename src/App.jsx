    ...
    <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:"16px" }}>
      <div style={{ background:"#1A1610",border:"1px solid #3A2E20",padding:"clamp(24px,5vw,48px)",width:"100%",maxWidth:"460px",fontFamily:"'Cormorant Garamond',serif" }}>
        ...
        <div style={{ marginTop:"36px",display:"flex",gap:"12px" }}>
          ...
          <button onClick={()=>{setDraft({...DEFAULT_RATES});setSaved(false);}} style={{ fontFamily:"'Jost',sans-serif",fontSize:"10px",letterSpacing:"2px",textTransform:"uppercase",background:"none",border:"1px solid #3A2E20",color:"#6B6058",padding:"14px 16px",cursor:"pointer" }}>Reset</button>
        </div>
      </div>
    </div>
  );
    ...
    <div style={{ fontFamily:"'Cormorant Garamond','Georgia',serif",background:"#F5F2EE",minHeight:"100vh",color:"#1A1610",overflowX:"hidden" }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .cond{font-family:'Jost',sans-serif;font-size:9px;letter-spacing:1px;text-transform:uppercase;background:#2C2420;color:#C9B89A;padding:3px 7px;position:absolute;top:10px;right:10px;z-index:1}

        .banner{background:#1A1610;color:#C9B89A;font-family:'Jost',sans-serif;font-size:10px;letter-spacing:3px;text-transform:uppercase;text-align:center;padding:9px 12px}
        ...
        @media(max-width:767px){
          ...
          .hero-text{padding:52px 24px !important}
          .promise-grid{grid-template-columns:1fr 1fr !important;gap:28px !important;padding:48px 24px !important}
          .footer-grid{grid-template-columns:1fr 1fr !important;gap:28px !important}
          .products-pad{padding:48px 16px !important}
          .filter-bar{flex-direction:column !important;gap:12px !important;padding:12px 16px !important}
          .filter-scroll{overflow-x:auto;-webkit-overflow-scrolling:touch;display:flex;gap:4px;padding-bottom:4px}
          .filter-scroll::-webkit-scrollbar{display:none}
          .cur-row{display:flex;gap:6px;flex-wrap:wrap}
        }
...
{/* Banner */}
      ...
      <nav style={{ display:"flex",alignItems:"center",justifyContent:"space-between",padding:isMobile?"16px 20px":"22px 48px",borderBottom:"1px solid #DDD5C8",position:"sticky",top:0,background:"#F5F2EE",zIndex:100 }}>
        ...
        <div style={{ display:"flex",gap:"12px",alignItems:"center" }}>
          ...
          <button onClick={()=>setShowCart(true)} style={{ fontFamily:"'Jost',sans-serif",fontWeight:300,fontSize:"11px",letterSpacing:"3px",textTransform:"uppercase",background:"none",border:"none",cursor:"pointer",color:"#1A1610",position:"relative" }}>
            🛒
            {cartQty>0 && (
              <span style={{ position:"absolute",top:"-8px",right:"-10px",background:"#7A5C3E",color:"#fff",borderRadius:"50%",width:"16px",height:"16px",fontSize:"9px",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Jost',sans-serif" }}>{cartQty}</span>
            )}
          </button>
        </div>
      </nav>
      ...
      {/* Hero */}
      <section className="hero-grid" style={{ display:"grid",gridTemplateColumns:"1fr 1fr",minHeight:"82vh",overflow:"hidden" }}>
      ...
      <section id="products" className="products-pad" style={{ padding:"64px 48px" }}>
        <div style={{ textAlign:"center",marginBottom:"48px" }}>
          <p style={{ fontFamily:"'Jost',sans-serif",fontSize:"10px",letterSpacing:"5px",textTransform:"uppercase",color:"#7A5C3E",marginBottom:"12px" }}>
            {activeCat==="All"?"All Products":activeCat}
          </p>
          <h2 style={{ fontSize:"clamp(26px,4vw,46px)",fontWeight:300,letterSpacing:"-0.3px" }}>
            {activeCat==="All"?"Shop Everything":`Browse ${activeCat}`}
          </h2>
          <div style={{ width:"36px",height:"1px",background:"#7A5C3E",margin:"16px auto 0" }}/>
        </div>

        <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(220px,1fr))",gap:"28px 16px",maxWidth:"1200px",margin:"0 auto" }}>
      ...
      {/* Why Us */}
      <section className="promise-grid" style={{ background:"#2C2420",padding:"64px 48px",display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:"36px",textAlign:"center" }}>
        {[
          { icon:"🚢", title:"Direct from China",  body:"No middlemen. Factory prices passed to you." },
          { icon:"🌍", title:"4-Country Delivery",  body:"ZA, Zimbabwe, Zambia & Botswana." },
          { icon:"✅", title:"Quality Checked",     body:"Every item inspected before shipping." },
          { icon:"📲", title:"Order via WhatsApp",  body:"Just tap — we handle the rest for you." },
        ].map(item=>(
          <div key={item.title}>
      ...
      {/* Footer */}
      <footer style={{ background:"#1A1610",padding:"48px clamp(20px,5vw,48px) 24px" }}>
        <div className="footer-grid" style={{ display:"grid",gridTemplateColumns:"2fr 1fr 1fr",gap:"40px",marginBottom:"36px" }}>
          <div>
            ...
            <a href={`https://wa.me/${WA_NUMBER}`} target="_blank" rel="noopener noreferrer"
              style={{ display:"inline-flex",alignItems:"center",gap:"8px",background:"#25D366",color:"#fff",fontFamily:"'Jost',sans-serif",fontSize:"10px",letterSpacing:"2px",textTransform:"uppercase",padding:"10px 18px",borderRadius:"2px" }}>
              📲 Chat with Us
            </a>
          </div>
          {[
            { title:"Shop",    links:["Crocs","iPhones","Android","Baby Clothes","Handbags"] },
            { title:"Support", links:["Shipping Info","Returns","WhatsApp Us","Track Order","FAQ"] },
          ].map(col=>(
            <div key={col.title}>
